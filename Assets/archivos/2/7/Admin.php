...
class Admin extends Controller
{
    ...
    public function subirCarpeta()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['files']) && isset($_POST['paths'])) {
            $id_carpeta = isset($_POST['from_home']) && $_POST['from_home'] === 'true' ? NULL : (!empty($_POST['id_carpeta']) ? intval($_POST['id_carpeta']) : NULL);
            $files = $_FILES['files'];
            $paths = $_POST['paths'];
    ...
    // Método corregido para subir archivos
    public function subirArchivo()
    {
        ...
        try {
            ...
            if (!file_exists($carpeta_final)) {
                if (!mkdir($carpeta_final, 0755, true)) {
                ...
                if (!empty($resultado_bd) && is_array($resultado_bd)) {
                    ...
                    if ($exito_bd && $id_archivo_nuevo > 0) {
                        ...
                        if (move_uploaded_file($tmp, $rutaDestino)) {
                            ...
                            // TODO: Considerar eliminar el registro de BD si no se pudo mover el archivo
                            // $this->model->eliminarArchivo($id_archivo_nuevo);
                        }
                    } else {
