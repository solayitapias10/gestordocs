...
class Archivos extends Controller
{
    ...
    public function index()
    {
        ...
        $data['menu'] = 'archivos';
        $archivos = $this->model->getArchivos($this->id_usuario);
    ...
    public function eliminarCarpeta($id)
    {
        ...
        if (empty($id) || !is_numeric($id) || $id <= 0) {
            $res = array('tipo' => 'warning', 'mensaje' => 'ID de carpeta no válido');
        } else {
            try {
                $data = $this->model->eliminarCarpeta($id, $this->id_usuario);
                if ($data > 0) {
                    $res = array('tipo' => 'success', 'mensaje' => 'Carpeta eliminada. Se borrará permanentemente en 30 días.');
                } else {
                    $res = array('tipo' => 'error', 'mensaje' => 'Error al eliminar la carpeta');
                }
            } catch (Exception $e) {
                error_log("Error al eliminar carpeta ID: $id, Usuario: {$this->id_usuario}. Error: " . $e->getMessage());
                $res = array('tipo' => 'error', 'mensaje' => 'No tienes permiso para eliminar esta carpeta o ya está eliminada');
            }
        }
        ob_end_clean();
        header('Content-Type: application/json');
                ...
                foreach ($archivos as $archivo) {
                    try {
                        if (isset($archivo['id_usuario']) && $archivo['id_usuario'] == $this->id_usuario) {
                            $result = $this->model->eliminarArchivoPermanente($archivo['id']);
                            if ($result == 0) {
                                $success = false;
