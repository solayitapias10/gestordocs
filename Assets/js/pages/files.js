    ...
    if (this.readyState == 4) {
      if (this.status == 200) {
        try {
          ...
          if (res.tipo === "success") {
            let html = "";
            const content_acordeon = document.querySelector(
              "#accordionFlushExample"
            );
            const container_archivos = document.querySelector(
              "#container-archivos"
            );
            const myModalUser = new bootstrap.Modal(
      ...
      if (this.status == 200) {
        try {
          ...
          if (res.tipo === "success" || res.id_carpeta !== undefined) {
            if (res.id_carpeta && res.id_carpeta !== "1") {
              ...
              const fileName =
                document
                  ...
                  ?.querySelector(".ver-archivo")?.textContent ||
            ...
            alertaPerzonalizada(
              "error",
              res.mensaje || "Archivo o carpeta no encontrado."
            );
          }
        } catch (e) {
          console.error(
        ...
        alertaPerzonalizada(
          "error",
          `Error ${this.status}: No se pudo cargar la carpeta.`
        );
      }
    }
  };
      ...
      if (this.status === 200) {
        try {
          ...
          if (res.tipo === "success") {
            ...
            const contenidoVisualizador = document.querySelector(
              "#contenidoVisualizador"
            );
            const modalVisualizadorLabel = document.querySelector(
              "#modalVisualizadorLabel"
            );
            const myModalVisualizador = new bootstrap.Modal(
              document.querySelector("#modalVisualizador")
            );
            ...
            if (tipo.includes("image")) {
              ...
              contenido = `<pre class="p-3">${
                res.contenido || "Vista previa no disponible"
              }</pre>`;
            } else {
    ...
    http.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        if (!user.error) {
          ...
          document.querySelector(".sidebar-user-switcher img").src = user.avatar
...
const editarArchivo = document.querySelectorAll(".editar-archivo");
  ...
  xhr.addEventListener("load", function () {
    ...
    if (this.status === 200) {
      try {
        ...
        console.error(
          "Respuesta del servidor (texto crudo):",
          this.responseText
        );
        onComplete(
          false,
          "Respuesta inválida del servidor. Consulta la consola para más detalles."
        );
        alertaPerzonalizada(
          "error",
          "Respuesta inválida del servidor. Consulta la consola para más detalles."
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (frmCarpeta) {
    frmCarpeta.addEventListener("submit", function (e) {
      ...
      if (id) {
        if (tipo === "archivo") {
          url = base_url + "admin/editarArchivo";
        } else {
  ...
  if (file) {
    file.addEventListener("change", async function (e) {
      ...
      if (totalSizeMB > 50) {
        alertaPerzonalizada(
          "warning",
          "Los archivos exceden el límite de 50MB. Selecciona archivos más pequeños."
        );
        e.target.value = "";
        return;
      }
  ...
  if (folder) {
    folder.addEventListener("change", async function (e) {
      ...
      if (files.length === 0) {
        alertaPerzonalizada(
          "warning",
          "No se seleccionaron archivos en la carpeta"
        );
        if (fileList) fileList.innerHTML = "";
        if (uploadStatus) uploadStatus.classList.add("d-none");
        return;
      ...
      if (totalSizeMB > 50 || files.length > 100) {
        alertaPerzonalizada(
          "warning",
          "La carpeta excede los límites (50MB o 100 archivos). Selecciona una carpeta más pequeña."
        );
        if (uploadStatus) uploadStatus.classList.add("d-none");
  ...
  editarCarpeta.forEach((enlace) => {
    enlace.addEventListener("click", function (e) {
      ...
      document
        .querySelector("#frmCarpeta")
        .setAttribute("data-tipo", "carpeta");
      btnCarpetaSubmit.textContent = "Actualizar";
      myModal1.show();
    });
  });

  // Configura el modal para editar un archivo.
  editarArchivo.forEach((enlace) => {
    enlace.addEventListener("click", function (e) {
      e.preventDefault();
      const id = e.target.getAttribute("data-id");
      const nombreActual = e.target.getAttribute("data-nombre");
      document.querySelector("#title-carpeta").textContent = "Editar Archivo";
      document.querySelector("#nombre").value = nombreActual;
      document.querySelector("#frmCarpeta").setAttribute("data-id", id);
      document
        .querySelector("#frmCarpeta")
        .setAttribute("data-tipo", "archivo");
      btnCarpetaSubmit.textContent = "Actualizar";
      myModal1.show();
    });
  });
